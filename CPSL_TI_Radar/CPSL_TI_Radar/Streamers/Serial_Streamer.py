...
class SerialStreamer(_Streamer):
    ...
    def _get_next_frame_packet(self):
        ...
        try:
            self.byte_buffer = self.serial_port.read_until(expected=self.magic_word)[
                :-8
            ]
        ...
        if packet_valid:
            try:
                ...
                self._conn_send_message_to_print(
                    "SerialStreamer._serial_get_next_packet: attempted to send new packet to Processor, but processor was closed"
                )
                self._conn_send_parent_error_message()
    ...
    def _serial_print_packet_header(self):
        ...
        self._conn_send_message_to_print(
            "detected packets: {}".format(self.detected_packets)
        )
        self._conn_send_message_to_print("\t version:{}".format(self.header["version"]))
        self._conn_send_message_to_print(
            "\t total packet length:{}".format(self.header["packet_length"])
        )
        self._conn_send_message_to_print(
            "\t platform:{}".format(self.header["platform"])
        )
        self._conn_send_message_to_print(
            "\t Frame Number:{}".format(self.header["frame_number"])
        )
        self._conn_send_message_to_print(
            "\t Time (CPU Cycles):{}".format(self.header["time"])
        )
        self._conn_send_message_to_print(
            "\t Number of Detected Objects:{}".format(
                self.header["num_detected_objects"]
            )
        )
        self._conn_send_message_to_print(
            "\t Number of Data Structures in package:{}".format(
                self.header["num_data_structures"]
            )
        )
