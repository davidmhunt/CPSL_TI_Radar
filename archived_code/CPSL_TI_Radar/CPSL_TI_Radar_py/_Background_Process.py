...
class _BackgroundProcess:
    def __init__(
        ...
        try:
            ...
            self._conn_send_message_to_print(
                "{}.__init__: could not find {} in {}".format(
                    self._process_name, settings_file_path, os.getcwd()
                )
            )
    ...
    def _serial_init_serial_port(
        self, address: str, baud_rate, timeout=1, close: bool = False
    ):
        ...
        try:
            ...
            self._conn_send_message_to_print(
                "{}.__init__:could not find serial port{}. Available ports are: {}".format(
                    self._process_name, address, available_ports
                )
            )
            self._conn_send_init_status(init_success=False)
            self.init_success = False
    ...
    def _conn_send_command_executed_message(self, command: _MessageTypes):
        ...
        self._conn_parent.send(
            _Message(type=_MessageTypes.COMMAND_EXECUTED, value=command)
        )
        return
